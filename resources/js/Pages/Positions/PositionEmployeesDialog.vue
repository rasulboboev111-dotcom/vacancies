...
watch(open, (isOpen) => {
    ...
    window.axios
        ...
        .catch((error) => {
            ...
            // как «у этой вазифы нет сотрудников»; логируем причину, чтобы сетевой сбой
            // и 500 не выглядели одинаково в консоли.
            console.error('Не удалось загрузить сотрудников вазифы:', error);
            failed.value = true;
        })
        .finally(() => {
            loading.value = false;
