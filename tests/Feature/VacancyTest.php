...
class VacancyTest extends TestCase
{
    ...
    public function test_vacancy_stores_openings_and_defaults_to_one(): void
    {
        // An explicit headcount is stored.
        $this->actingAs($this->admin)->post(route('vacancies.store'), [
            'branch_id' => $this->branch1->id,
            'title' => 'Multi Hire',
            'openings' => 3,
        ])->assertRedirect();
        $this->assertDatabaseHas('vacancies', ['title' => 'Multi Hire', 'openings' => 3]);

        // Omitting it defaults to one opening.
        $this->actingAs($this->admin)->post(route('vacancies.store'), [
            'branch_id' => $this->branch1->id,
            'title' => 'Single Hire',
        ])->assertRedirect();
        $this->assertDatabaseHas('vacancies', ['title' => 'Single Hire', 'openings' => 1]);

        // A non-positive headcount is rejected.
        $this->actingAs($this->admin)
            ->from(route('vacancies.index'))
    ...
    public function test_admin_can_create_vacancy(): void
    {
        ...
        // Creating from the default "all branches" view (no filter passed)
        // redirects back to the unfiltered list, so the new vacancy stays
        // visible instead of narrowing the list to its branch.
    ...
    public function test_free_text_position_is_resolved_and_reused(): void
    {
        // A brand-new position name is created on the fly and linked.
        $this->actingAs($this->admin)->post(route('vacancies.store'), [
            'branch_id' => $this->branch1->id,
        ...
        $position = Position::where('name', 'Ведущий инженер')->first();
        ...
        // The same name (case-insensitively) reuses the row, not a duplicate.
        $this->actingAs($this->admin)->post(route('vacancies.store'), [
            'branch_id' => $this->branch1->id,
        ...
        $this->assertSame(1, Position::where('name', 'Ведущий инженер')->count());
    }
    ...
    public function test_admin_toggle_keeps_the_selected_branch_filter(): void
    {
        $vacancy = Vacancy::create([
            'branch_id' => $this->branch1->id,
            'title' => 'Filtered',
        ...
        // No filter passed → redirect to the unfiltered list (all branches),
        // so toggling never hides vacancies from other branches.
        $this->actingAs($this->admin)
        ...
        // Filter passed → it is preserved in the redirect, not replaced by the
        // vacancy's own branch.
        $this->actingAs($this->admin)
            ->put(route('vacancies.update', ['vacancy' => $vacancy->id, 'filter_branch_id' => $this->branch2->id]), [
                'branch_id' => $this->branch1->id,
                'title' => 'Filtered',
                'status' => 'open',
            ])
            ->assertRedirect(route('vacancies.index', ['filter' => ['branch_id' => $this->branch2->id]]));
    }
    ...
    public function test_admin_can_filter_vacancies_by_branch_and_status(): void
    {
        Vacancy::create(['branch_id' => $this->branch1->id, 'title' => 'B1 Open', 'status' => 'open']);
        Vacancy::create(['branch_id' => $this->branch1->id, 'title' => 'B1 Closed', 'status' => 'closed']);
        Vacancy::create(['branch_id' => $this->branch2->id, 'title' => 'B2 Open', 'status' => 'open']);

        $this->actingAs($this->admin)
            ->get(route('vacancies.index', ['filter' => ['branch_id' => $this->branch1->id]]))
            ->assertInertia(fn ($page) => $page->has('vacancies', 2));

        $this->actingAs($this->admin)
            ->get(route('vacancies.index', ['filter' => ['branch_id' => $this->branch1->id, 'status' => 'open']]))
            ->assertInertia(fn ($page) => $page
                ->has('vacancies', 1)
                ->where('vacancies.0.title', 'B1 Open'));
    }
