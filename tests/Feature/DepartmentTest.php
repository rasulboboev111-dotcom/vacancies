...
class DepartmentTest extends TestCase
{
    ...
    public function test_deleting_department_detaches_employees_and_vacancies(): void
    {
        ...
        $position = Position::create(['name' => 'Clerk']);
    ...
    public function test_cycle_through_soft_deleted_department_is_rejected(): void
    {
        // grandparent → parent → child, then archive the intermediate parent.
        // Re-parenting grandparent under child still closes a cycle; the check
        // must see through the soft-deleted node (withTrashed traversal).
        $grandparent = Department::create(['branch_id' => $this->branch1->id, 'name' => 'Grandparent']);
        $parent = Department::create(['branch_id' => $this->branch1->id, 'parent_id' => $grandparent->id, 'name' => 'Mid']);
        $child = Department::create(['branch_id' => $this->branch1->id, 'parent_id' => $parent->id, 'name' => 'Leaf']);
        $parent->delete();

        $this->actingAs($this->admin)->put(route('departments.update', $grandparent), [
            'branch_id' => $this->branch1->id,
            'parent_id' => $child->id,
            'name' => 'Grandparent',
    ...
    public function test_department_with_soft_deleted_child_cannot_be_deleted(): void
    {
        $parent = Department::create(['branch_id' => $this->branch1->id, 'name' => 'Parent']);
        $child = Department::create(['branch_id' => $this->branch1->id, 'parent_id' => $parent->id, 'name' => 'Archived Child']);
        $child->delete(); // only child is in the trash

        $this->actingAs($this->admin)
            ->delete(route('departments.destroy', $parent))
            ->assertSessionHasErrors('message');

        // Parent must stay (deleting it would dangle the trashed child's parent_id).
        $this->assertDatabaseHas('departments', ['id' => $parent->id, 'deleted_at' => null]);
    }
}
